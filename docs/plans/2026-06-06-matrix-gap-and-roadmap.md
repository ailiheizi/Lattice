---
title: "design: NextIM vs Matrix 能力差距与路线图"
type: design
date: 2026-06-06
status: living
---

# NextIM vs Matrix 能力差距与路线图

本文以 Matrix 为成熟参照系,记录 NextIM 当前能力、与 Matrix 的差距、以及推进路线。
...
| 能力 | 状态 | 证据 |
|------|------|------|
| 1v1/群聊消息收发、存储、同步 | ✅ | `nextim-store/src/server.rs`,集成测试 |
| 房间成员管理(建房/加入/退出/踢人 + RoomEvent) | ✅ | `nextim-core/src/room.rs` |
...
| 全文搜索(Tantivy,中文分词) | ✅ | `nextim-storage/src/tantivy_search.rs` |
...
- **方案 A(本轮采用)**:接线 `nextim-core::exchange::ExchangePolicy`——server 收消息时调
...
- 实现细节以代码 + `.plans/nextim-dev/docs/` 控制面为准。
