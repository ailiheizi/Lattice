---
title: "feat: NextIM 去中心化 IM 技术实现计划"
type: feat
date: 2026-03-18
---

# NextIM 去中心化 IM 技术实现计划

## Overview

基于 brainstorm 阶段确定的架构设计，本文档定义 NextIM 的 Rust 项目结构、核心 Trait、Protobuf schema、开发里程碑。
...
│   │   ├── build.rs
│   │   └── src/lib.rs
│   ├── nextim-crypto/            # 加密层 (vodozemac 封装)
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── identity.rs       # 身份密钥管理
...
members = [
    "crates/nextim-proto",
    "crates/nextim-crypto",
    "crates/nextim-core",
    "crates/nextim-transport",
    "crates/nextim-storage",
    "crates/nextim-store",
    "crates/nextim-peer",
    "crates/nextim-ffi",
]
...
nextim-ffi
    ↓
nextim-store / nextim-peer        (二进制目标)
    ↓              ↓
nextim-core                       (核心业务逻辑, sans-I/O)
    ↓       ↓          ↓
...
transport  storage     crypto
    ↓       ↓          ↓
nextim-proto                      (Protobuf 类型定义)
```

规则：
- `nextim-core` 只定义 Trait，不依赖任何具体实现
- `nextim-transport`、`nextim-storage`、`nextim-crypto` 实现 Trait
- `nextim-store` 和 `nextim-peer` 是二进制入口，组装所有依赖
- `nextim-ffi` 封装 `nextim-core` + 实现层，暴露给 Android/iOS
...
```rust
// crates/nextim-core/src/traits/transport.rs
...
```rust
// crates/nextim-core/src/traits/storage.rs

use crate::error::Result;
use nextim_proto::{Message, Room, Contact, DeviceInfo, KeyBundle};
...
```rust
// crates/nextim-core/src/traits/search.rs
...
- [ ] `nextim-proto`: 配置 prost-build，`cargo build` 生成 Rust 类型
- [ ] `nextim-core`: 定义 Transport、Storage、SearchIndex Trait
- [ ] `nextim-core`: 定义 Error 类型（thiserror）
- [ ] 验证：`cargo check --workspace` 通过

**产出文件：**
- `Cargo.toml` (workspace root)
- `proto/*.proto` (4 个文件)
- `crates/nextim-proto/` (build.rs + lib.rs)
- `crates/nextim-core/src/traits/` (transport.rs, storage.rs, search.rs)
- `crates/nextim-core/src/error.rs`

### Phase 2: 身份与签名（安全基础）

**目标：** 用户可以生成密钥对、创建身份、对消息签名和验证。

- [ ] `nextim-crypto/src/identity.rs`: 生成 Ed25519 + Curve25519 密钥对
- [ ] `nextim-crypto/src/identity.rs`: 导出/导入私钥（加密存储）
- [ ] `nextim-crypto/src/identity.rs`: 生成 SHA-256 指纹
- [ ] `nextim-crypto/src/sign.rs`: 消息签名（Ed25519）
- [ ] `nextim-crypto/src/sign.rs`: 签名验证
- [ ] `nextim-crypto/src/sign.rs`: SHA-256 完整性校验
- [ ] `nextim-crypto/src/trust.rs`: 三档信任模型（Public/TOFU/Verified）
- [ ] `nextim-crypto/src/identity.rs`: 设备密钥生成 + 主密钥签名
- [ ] 单元测试：密钥生成、签名验证、信任判断
- [ ] 身份卡片序列化/反序列化（IdentityCard protobuf）

**产出文件：**
- `crates/nextim-crypto/src/` (identity.rs, sign.rs, trust.rs)

### Phase 3: 存储层实现

**目标：** 消息可以持久化存储和全文搜索。

- [ ] `nextim-storage/src/sqlite.rs`: 实现 Storage Trait
- [ ] SQLite schema 建表（messages, rooms, contacts, devices, keys）
- [ ] SQLite 索引优化（room_id + timestamp 复合索引）
- [ ] `nextim-storage/src/tantivy.rs`: 实现 SearchIndex Trait
...
**产出文件：**
- `crates/nextim-storage/src/` (sqlite.rs, tantivy.rs, memory.rs)

### Phase 4: 传输层实现

**目标：** 两个 Store 节点可以通过 WebSocket 互发 Protobuf 消息。

- [ ] `nextim-transport/src/websocket.rs`: 实现 Transport Trait（客户端）
- [ ] `nextim-transport/src/websocket.rs`: 实现 TransportListener Trait（服务端）
...
**产出文件：**
- `crates/nextim-transport/src/websocket.rs`

### Phase 5: Store 节点

**目标：** 可运行的 Store 节点，支持消息收发、存储、搜索。

- [ ] `nextim-store/src/server.rs`: WebSocket 服务端，接受连接
- [ ] `nextim-store/src/relay.rs`: 消息路由（本地存储 + 转发给接收方）
- [ ] `nextim-store/src/api.rs`: RESTful API（手机端查询消息、发送消息、搜索）
- [ ] 消息投递流程：直连 → 失败记录待发
- [ ] 联系人管理（添加、删除、信任等级）
...
**产出文件：**
- `crates/nextim-store/src/` (main.rs, server.rs, relay.rs, api.rs)

### Phase 6: E2EE 加密（可选层）

**目标：** 用户可以开启 1v1 和群聊的端到端加密。

- [ ] `nextim-crypto/src/olm.rs`: 封装 vodozemac Olm（1v1 加密会话）
- [ ] One-time key 生成和管理
- [ ] Olm 会话建立（3DH 密钥交换）
- [ ] `nextim-crypto/src/megolm.rs`: 封装 vodozemac Megolm（群聊加密）
...
**产出文件：**
- `crates/nextim-crypto/src/` (olm.rs, megolm.rs)

### Phase 7: 群聊与频道

**目标：** 完整的群聊功能，包括权限管理和公开频道。

- [ ] `nextim-core/src/room.rs`: 群组创建、成员管理
...
**产出文件：**
- `crates/nextim-core/src/room.rs` 完善

### Phase 8: Peer 中转节点

**目标：** Peer 节点可以中转消息，支持短时缓存和代收转投。

- [ ] `nextim-peer/src/relay.rs`: 消息中转逻辑
- [ ] `nextim-peer/src/cache.rs`: 短时缓存（内存，TTL + 容量上限）
- [ ] `nextim-peer/src/stun.rs`: STUN/NAT 穿透基础
- [ ] 超时未送达 → 转投代收 Store
- [ ] Peer 节点配置（缓存大小、TTL、监听地址）

**产出文件：**
- `crates/nextim-peer/src/` (main.rs, relay.rs, cache.rs, stun.rs)

### Phase 9: Android FFI

**目标：** Rust 核心通过 UniFFI 暴露给 Android/Kotlin。

- [ ] `nextim-ffi`: UniFFI 配置，定义暴露的接口
...
**产出文件：**
- `crates/nextim-ffi/` (lib.rs, uniffi.toml)

### Phase 10: 信息交换端 + 多设备同步

**目标：** 用户可以管理消息接收策略，多设备同步消息。

- [ ] `nextim-core/src/exchange.rs`: 信息交换端逻辑
- [ ] 接收策略管理（按联系人/群组设置信任等级）
- [ ] 消息过滤（根据信任等级决定是否接收）
- [ ] `nextim-core/src/device.rs`: 多设备同步
- [ ] 设备注册/注销
- [ ] E2EE 密钥跨设备同步（加密存储在 Store）

**产出文件：**
- `crates/nextim-core/src/` (exchange.rs, device.rs)
...
- `docs/brainstorms/2026-03-17-nextim-architecture-brainstorm.md` — 架构设计
