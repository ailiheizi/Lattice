...
- **E5**:轮换(成员变更触发)+ 多设备分发。✅ 已落地:`MegolmSessionManager::rotate(room_id)`(废弃旧出站会话+生成新 session_key)+ `key_distribution::distribute_to_devices`(批量对多设备 Olm 加密分发)。2 单测:轮换后旧成员无法解新消息(前向保密)、多设备批量分发全部可解。`KeyRotationPolicy::should_rotate` 提供阈值判定。
