uniffi::include_scaffolding!("lattice");
...
impl From<lattice_core::error::LatticeError> for LatticeFfiError {
    fn from(e: lattice_core::error::LatticeError) -> Self {
...
impl LatticeClient {
    pub fn new(data_dir: String) -> Result<Self, LatticeFfiError> {
        let path = PathBuf::from(&data_dir);
        std::fs::create_dir_all(&path).map_err(|e| LatticeFfiError::StorageError(e.to_string()))?;

        let storage = SqliteStorage::open(path.join("store.db"))
            .map_err(|e| LatticeFfiError::StorageError(e.to_string()))?;
        let search = TantivySearch::open(path.join("search_index"))
            .map_err(|e| LatticeFfiError::StorageError(e.to_string()))?;

        let rt =
            tokio::runtime::Runtime::new().map_err(|e| LatticeFfiError::Other(e.to_string()))?;
    ...
    ) -> Result<FfiIdentityCard, LatticeFfiError> {
    ...
    pub fn sign_data(&self, data: Vec<u8>) -> Result<Vec<u8>, LatticeFfiError> {
    ...
    pub fn verify_signature(
        ...
        data: Vec<u8>,
        signature: Vec<u8>,
    ) -> Result<bool, LatticeFfiError> {
        use ed25519_dalek::{Signature, Verifier, VerifyingKey};

        let key_bytes: [u8; 32] = public_key
            .as_slice()
            .try_into()
            .map_err(|_| LatticeFfiError::CryptoError("invalid key length".into()))?;
        let verifying_key = VerifyingKey::from_bytes(&key_bytes)
            .map_err(|e| LatticeFfiError::CryptoError(e.to_string()))?;

        let sig_bytes: [u8; 64] = signature
            .as_slice()
            .try_into()
            .map_err(|_| LatticeFfiError::CryptoError("invalid signature length".into()))?;
    ...
    pub fn send_message(&self, room_id: String, text: String) -> Result<String, LatticeFfiError> {
    ...
    pub fn get_messages(
        ...
        until: u64,
        limit: u32,
    ) -> Result<Vec<FfiMessage>, LatticeFfiError> {
        self.rt.block_on(async {
            let range = TimeRange {
                start: since,
                end: until,
    ...
    pub fn search(
        ...
        query: String,
        limit: u32,
    ) -> Result<Vec<FfiSearchResult>, LatticeFfiError> {
        self.rt.block_on(async {
            let results = self
                .search
...
mod tests {
    ...
    fn make_client() -> (LatticeClient, tempfile::TempDir) {
        let dir = tempfile::tempdir().unwrap();
        let client = LatticeClient::new(dir.path().to_str().unwrap().to_string()).unwrap();
