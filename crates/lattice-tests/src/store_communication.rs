//! Integration tests for real `lattice-store` WebSocket handling.
...
use lattice_store::{server, AppState, OnlineConnections, OutboundPool};
...
async fn real_ws_server_stores_and_syncs_room_events() {
    ...
    if let Some(frame::Body::SyncResponse(response)) = sync_frame.body {
        ...
        match &item.item {
            Some(lattice_proto::transport::sync_timeline_item::Item::RoomEvent(ev)) => {
...
async fn real_ws_server_unifies_messages_and_room_events_in_timeline() {
    ...
    let stored_msg = lattice_proto::message::Message {
    ...
    if let Some(frame::Body::SyncResponse(response)) = sync_frame.body {
        ...
        for item in &response.timeline {
            ...
            match &item.item {
                Some(lattice_proto::transport::sync_timeline_item::Item::Message(_)) => {
                    has_message = true
                }
                Some(lattice_proto::transport::sync_timeline_item::Item::RoomEvent(_)) => {
                    has_event = true
                }
