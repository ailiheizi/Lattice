...
use lattice_core::traits::transport::Transport;
...
impl Transport for WsTransport {
    async fn connect(&mut self, addr: &str) -> Result<()> {
        let (ws_stream, _) = connect_async(addr)
            .await
            .map_err(|e| LatticeError::Transport(format!("connect failed: {e}")))?;
    ...
    async fn send(&self, data: &[u8]) -> Result<()> {
        let stream = self
            .stream
            .as_ref()
            .ok_or_else(|| LatticeError::Transport("not connected".into()))?;

        let mut guard = stream.lock().await;
        guard
            .send(WsMessage::Binary(data.to_vec()))
            .await
            .map_err(|e| LatticeError::Transport(format!("send failed: {e}")))?;
        Ok(())
    }
        ...
        loop {
            match guard.next().await {
                ...
                Some(Ok(WsMessage::Close(_))) => {
                    ...
                    return Err(LatticeError::Transport("connection closed".into()));
                }
                Some(Ok(_)) => continue, // 忽略 Text/Pong 等
                Some(Err(e)) => {
                    self.connected.store(false, Ordering::SeqCst);
                    return Err(LatticeError::Transport(format!("recv error: {e}")));
                }
                None => {
                    self.connected.store(false, Ordering::SeqCst);
                    return Err(LatticeError::Transport("stream ended".into()));
                }
            }
        }
    }
...
use lattice_core::traits::transport::TransportListener;
...
impl TransportListener for WsListener {
    ...
    async fn bind(addr: &str) -> Result<Self> {
        let listener = TcpListener::bind(addr)
            .await
            .map_err(|e| LatticeError::Transport(format!("bind failed: {e}")))?;
        Ok(Self { listener })
    }
    ...
    async fn accept(&mut self) -> Result<Self::Conn> {
        let (tcp_stream, _addr) = self
            ...
            .accept()
            .await
            .map_err(|e| LatticeError::Transport(format!("accept failed: {e}")))?;

        let ws_stream = accept_async(MaybeTlsStream::Plain(tcp_stream))
            .await
            .map_err(|e| LatticeError::Transport(format!("ws handshake failed: {e}")))?;
...
mod tests {
    ...
    async fn test_protobuf_frame_roundtrip() {
        use lattice_proto::transport::{Frame, FrameType, Ping};
        use prost::Message;
        ...
        let frame = Frame {
            ...
            body: Some(lattice_proto::transport::frame::Body::Ping(Ping {
