...
impl TantivySearch {
    ...
    pub fn open(path: impl AsRef<Path>) -> Result<Self> {
        ...
        std::fs::create_dir_all(path.as_ref()).map_err(|e| LatticeError::Search(e.to_string()))?;
        let dir = tantivy::directory::MmapDirectory::open(path)
            .map_err(|e| LatticeError::Search(e.to_string()))?;
...
mod tests {
    use super::*;
    use lattice_proto::message::{MessageContent, MessageType};
