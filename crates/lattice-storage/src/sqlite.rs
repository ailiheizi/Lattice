...
use lattice_core::traits::storage::{Pagination, PendingMessage, Storage, TimeRange};
use lattice_proto::group::{Room, RoomEvent};
use lattice_proto::identity::{Contact, DeviceInfo, KeyBundle};
use lattice_proto::message::{Message, Reaction, ReadReceipt};
...
impl SqliteStorage {
    pub fn open(path: impl AsRef<Path>) -> Result<Self> {
        let conn = Connection::open(path).map_err(|e| LatticeError::Storage(e.to_string()))?;
        let storage = Self {
            conn: Mutex::new(conn),
        };
        storage.init_tables()?;
...
impl Storage for SqliteStorage {
    ...
    async fn get_messages(
        ...
        for row in rows {
            let data = row.map_err(|e| LatticeError::Storage(e.to_string()))?;
    ...
    async fn get_message_parents(&self, child_hash: &[u8]) -> Result<Vec<Vec<u8>>> {
        ...
        for row in rows {
            parents.push(row.map_err(|e| LatticeError::Storage(e.to_string()))?);
        }
        Ok(parents)
    }
    ...
    async fn get_head_message_hashes(&self) -> Result<Vec<Vec<u8>>> {
        ...
        for row in rows {
            heads.push(row.map_err(|e| LatticeError::Storage(e.to_string()))?);
        }
        Ok(heads)
    }
    ...
    async fn list_pending_messages(&self) -> Result<Vec<PendingMessage>> {
        ...
        let mut messages = Vec::new();
        for row in rows {
            messages.push(row.map_err(|e| LatticeError::Storage(e.to_string()))?);
        }
        Ok(messages)
    }
    ...
    async fn get_room_events_since(
        ...
        room_id: &str,
        since_received_ts: u64,
    ) -> Result<Vec<lattice_core::traits::storage::RoomEventRecord>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| LatticeError::Storage(e.to_string()))?;
        ...
        for row in rows {
            let (received_ts, data) = row.map_err(|e| LatticeError::Storage(e.to_string()))?;
            let event = RoomEvent::decode(data.as_slice())
                .map_err(|e| LatticeError::Serialization(e.to_string()))?;
            records.push(lattice_core::traits::storage::RoomEventRecord {
                event,
...
fn encode_hash_list(hashes: &[Vec<u8>]) -> Result<Vec<u8>> {
    ...
    for hash in hashes {
        let len = u32::try_from(hash.len())
            .map_err(|_| LatticeError::Storage("prev_hash is too large to encode".to_string()))?;
...
mod tests {
    ...
    fn make_encrypted_message(id: &str, room: &str, ts: u64) -> Message {
        Message {
            ...
            encrypted: true,
            verified: false,
            encrypted_payload: Some(lattice_proto::message::EncryptedPayload {
    ...
    async fn test_contact_crud() {
        use lattice_proto::identity::{Identity, TrustLevel};
