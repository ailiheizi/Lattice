...
use lattice_proto::{discovery::IdentityCard, message::Envelope};
...
pub fn compute_room_event_hash(
    event: &lattice_proto::group::RoomEvent,
...
pub fn compute_message_op_hash(
    op: &lattice_proto::message::MessageOp,
) -> Result<Vec<u8>, SignVerifyError> {
    let mut encoded = Vec::new();
...
pub fn compute_reaction_hash(
    reaction: &lattice_proto::message::Reaction,
) -> Result<Vec<u8>, SignVerifyError> {
    let mut encoded = Vec::new();
...
pub fn compute_read_receipt_hash(
    receipt: &lattice_proto::message::ReadReceipt,
) -> Result<Vec<u8>, SignVerifyError> {
    let mut encoded = Vec::new();
...
pub fn compute_typing_hash(
    typing: &lattice_proto::message::Typing,
) -> Result<Vec<u8>, SignVerifyError> {
    let mut encoded = Vec::new();
...
pub fn verify_sync_request_identity(
    req: &lattice_proto::transport::SyncRequest,
) -> Result<bool, SignVerifyError> {
    // (1) 公钥与声称指纹绑定
...
fn extract_payload_bytes(envelope: &Envelope) -> Result<Vec<u8>, SignVerifyError> {
    ...
    match &envelope.payload {
        Some(lattice_proto::message::envelope::Payload::Plain(p)) => {
            let mut buf = Vec::new();
            p.encode(&mut buf)
                .map_err(|e| SignVerifyError::SerializationError(e.to_string()))?;
            Ok(buf)
        }
        Some(lattice_proto::message::envelope::Payload::Encrypted(e)) => {
            let mut buf = Vec::new();
            e.encode(&mut buf)
                .map_err(|e| SignVerifyError::SerializationError(e.to_string()))?;
            Ok(buf)
...
mod tests {
    ...
    use lattice_proto::message::{envelope::Payload, MessageContent, MessageType, PlainPayload};
    use rand::rngs::OsRng;
    ...
    fn make_signed_room_event(signing_key: &SigningKey) -> lattice_proto::group::RoomEvent {
        use ed25519_dalek::Signer;
        let mut event = lattice_proto::group::RoomEvent {
    ...
    fn make_signed_message_op(key: &SigningKey) -> lattice_proto::message::MessageOp {
        let mut op = lattice_proto::message::MessageOp {
    ...
    fn make_signed_reaction(key: &SigningKey) -> lattice_proto::message::Reaction {
        let mut r = lattice_proto::message::Reaction {
    ...
    fn make_signed_receipt(key: &SigningKey) -> lattice_proto::message::ReadReceipt {
        let mut r = lattice_proto::message::ReadReceipt {
            room_id: "room-1".to_string(),
